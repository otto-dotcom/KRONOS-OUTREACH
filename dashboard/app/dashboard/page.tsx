    ...
    <div className="flex gap-1">
      {[7, 30, 90].map((d) => (
        <button
          key={d}
          onClick={() => onChange(d)}
          className={`px-3 py-1 text-[10px] tracking-wider transition-all cursor-pointer border ${
            days === d
...
function CampaignLauncher() {
  ...
  async function handleLaunch() {
    ...
    try {
      ...
      if (res.ok) {
        setResult({ ok: true, message: `Deployed // ${leadLimit} leads // EMAIL + SMS` });
      } else {
        setResult({ ok: false, message: data.error || "Launch failed" });
      }
    } catch {
  ...
  return (
    <div className="bg-[#0D0D0D] border border-[#1A1A1A] p-6 glow">
      ...
      {result && (
        <div className={`mt-4 flex items-center gap-2 p-3 border fade-up ${
          result.ok ? "border-green-900/50 bg-green-950/20" : "border-red-900/50 bg-red-950/20"
        }`}>
        ...
        <div className="mt-4 space-y-3 fade-up">
          ...
          <div className="flex items-center gap-2 p-2 border border-[#1A1A1A] bg-[#060606]">
            ...
            <span className="text-[9px] text-[#888] tracking-wider uppercase">
              AI-Generated via GPT-4o-mini // Personalized per lead
            </span>
          </div>
  ...
  return (
    <div className="bg-[#0D0D0D] border border-[#1A1A1A] p-6">
      ...
      {loading ? (
        ...
        /* SendGrid API key needs stats scope */
        <div className="py-6 space-y-4">
          <div className="flex items-center gap-3 p-4 border border-[#FF6B00]/30 bg-[#FF6B00]/5">
            <IconWarn />
            <div>
              <p className="text-xs text-[#FF6B00] font-semibold tracking-wider">API KEY NEEDS UPDATE</p>
              <p className="text-[10px] text-[#AAA] mt-1 leading-relaxed">
                Your SendGrid key is missing the &quot;Stats Read&quot; permission.
              </p>
            </div>
          </div>
          <div className="bg-[#0A0A0A] border border-[#161616] p-4 space-y-2">
            <p className="text-[10px] text-[#999] tracking-wider uppercase font-semibold">Fix in 3 steps:</p>
            <div className="space-y-1.5">
              {[
                "Go to SendGrid > Settings > API Keys",
                "Edit your key or create a new one",
                "Enable 'Stats' > 'Read Access' permission",
              ].map((step, i) => (
                <div key={i} className="flex items-start gap-2">
                  <span className="text-[10px] text-[#FF6B00] font-bold w-4">{i + 1}.</span>
                  <span className="text-[11px] text-[#AAA]">{step}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                {data.recent.map((msg, i) => (
                  <div key={i} className="flex items-start gap-2 py-1.5 border-b border-[#111] last:border-0">
                    <div className={`w-1.5 h-1.5 mt-1.5 shrink-0 ${
                      msg.status === "delivered" ? "bg-green-500" : "bg-red-500"
                    }`} />
